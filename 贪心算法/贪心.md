...
遇到一段持续上升的子序列时选择最后一个即可，这样才有更大的机会摆动
...
其实只需要记录每个位置可跳到的最远位置就行了，知道跳出或者跳不到当前可跳到的最远位置
...
- 再从后向前遍历，如果第i个孩子评分大于右边的，这时候他的糖果理应比右边多一个，那么他和左边孩子就有两种情况：1，评分比左边孩子高，那么第一趟比较的时候已经确定他的糖果比左边多了，取当前糖果和右边孩子糖果+1中较大的一个就能保证他比左右糖果都多；2，评分比左边孩子低，那么不用关心左边，同样可以用当前糖果和右边孩子糖果+1中较大的一个来保证他比右边孩子糖果多。
...
- 接下来只需要从前往后插入就行了，当前已排好队的数量不大于k值时直接插入就行，负责需要插入到下标为k的位置
